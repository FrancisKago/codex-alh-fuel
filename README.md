...
- Offline indicator on the dashboard showing connection state
...
synchronize them once connectivity is restored. High level helpers like
`createRequest` and `recordTransaction` automatically fall back to local
storage when a network error occurs.
